# Capture synthesis remains owned by Meeting Intelligence

Meeting Intelligence accepts a capture-set Observation containing only exact Logical Meeting capture identities, reads source material through guarded owned capabilities, and retains a separately versioned Luma Synthesis behind its existing `observe`, `query`, and `conclude` Interface. LogicalMeetings continues to own capture binding and provider ledgers continue to own original material; a private synthesis revision references both rather than becoming another raw capture or fabricated transcript.
