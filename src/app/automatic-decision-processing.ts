...
export async function createAutomaticDecisionProcessing(input: {
  ...
  await database.exec(`CREATE TABLE IF NOT EXISTS automatic_decision_jobs (
    ...
    ALTER TABLE automatic_decision_jobs ADD COLUMN IF NOT EXISTS attempt_observation_id TEXT;`);
  ...
  const settleJob = async (jobId: string, operation: () => Promise<void>) => {
    const prior = settlements.get(jobId) ?? Promise.resolve();
    let release = () => {};
    const current = new Promise<void>((resolve) => {
      release = resolve;
    });
    settlements.set(jobId, current);
    await prior;
    try {
      await operation();
    } finally {
      release();
      if (settlements.get(jobId) === current) settlements.delete(jobId);
    }
  };
    ...
    running = (async () => {
      while (active && !closed && interruptedCursor !== null) {
        const recovered = await database.query<{
          ...
          "SELECT job_id,payload_json,payload_hash,batch_id,attempt_observation_id,created_at::float8 AS created_at FROM automatic_decision_jobs WHERE workspace_id=$1 AND phase='interrupted' AND created_at>$2 ORDER BY created_at LIMIT 100",
          [workspace.workspaceId, interruptedCursor]
        );
        for (const row of recovered.rows) {
          if (!active || closed) break;
          interruptedCursor = row.created_at;
          await settleJob(row.job_id, async () => {
            try {
              ...
              const originalAttempt =
                row.attempt_observation_id ??
                (retry?.attempts === 1
                  ? `source-job:${row.job_id}`
                  : `source-job:${row.job_id}:retry:${(retry?.attempts ?? 0) - 1}`);
              if (
                retry &&
                originalAttempt === retry.lastObservationId &&
                ((retry.canRetry && retry.disposition === "not-dispatched") ||
                  retry.disposition === "completed")
              )
        ...
        await settleJob(row.job_id, async () => {
          ...
          try {
            ...
            const observationId = retained
              ...
              : `source-job:${row.job_id}`;
            await markAttempt(row.job_id, observationId);
            ...
            observed = true;
            await saveAttemptOutcome(row.job_id, batch);
          } catch {
            await database.query(
              "UPDATE automatic_decision_jobs SET phase=$3,retry_at=NULL WHERE workspace_id=$1 AND job_id=$2 AND phase='processing'",
              [
                workspace.workspaceId,
                row.job_id,
                observed ? "interrupted" : "unavailable"
              ]
            );
            if (observed) {
              // Reconcile the retained proof after a transient receipt fault
              // even if this process stays alive. A failed reproof is not looped.
              interruptedCursor = 0;
              wakePending = true;
            }
          }
  ...
  const markAttempt = (jobId: string, observationId: string) =>
    database.query(
      "UPDATE automatic_decision_jobs SET phase='processing',retry_at=NULL,attempt_observation_id=$3 WHERE workspace_id=$1 AND job_id=$2",
      [workspace.workspaceId, jobId, observationId]
    );
  const interruptAttempt = async (jobId: string) => {
    await database.query(
      "UPDATE automatic_decision_jobs SET phase='interrupted',retry_at=NULL WHERE workspace_id=$1 AND job_id=$2 AND phase='processing'",
      [workspace.workspaceId, jobId]
    );
    interruptedCursor = 0;
    schedule();
  };
  const saveAttemptOutcome = async (jobId: string, batch: AutomaticDecisionBatch) => {
    try {
      await saveOutcome(jobId, batch);
    } catch (error) {
      await interruptAttempt(jobId);
      throw error;
    }
  };
  ...
  const api = {
    ...
    retry(subject: DecisionSubject, observationId: string): Promise<void> {
      ...
      const task = (async () => {
        const rows = await database.query<{ job_id: string }>(
          "SELECT job_id FROM automatic_decision_jobs WHERE workspace_id=$1 AND subject_hash=$2 ORDER BY created_at DESC LIMIT 1",
        ...
        await settleJob(row.job_id, async () => {
          ...
          if (
            !head ||
            head.job_id !== row.job_id ||
            (head.batch_id && head.batch_id !== before.batch.batchId)
          )
            throw new Error("The retry notification changed");
          await markAttempt(row.job_id, observationId);
          let batch: AutomaticDecisionBatch;
          try {
            batch = await input.meetingIntelligence.observe({
              workspace,
              subject,
              observations: [
                {
            ...
            await interruptAttempt(row.job_id);
            throw error;
          }
          await saveAttemptOutcome(row.job_id, batch);
          schedule();
