...
export interface DecisionRecordCatalog {
  ...
  discover(
    this: void,
    input: { audience: DecisionAudience; limit: number }
  ): Promise<DecisionCatalogSnapshot>;
  requireCurrent(
    this: void,
    input: { audience: DecisionAudience; snapshot: DecisionCatalogSnapshot }
  ): Promise<void>;
  /** Untrusted logical/provider identifier: resolve uniquely against a complete catalog. */
  read(
    this: void,
    input: { audience: DecisionAudience; recordId: string }
  ...
  readReference(
    this: void,
    input: { audience: DecisionAudience; reference: ExternalReference }
  ): Promise<CanonicalDecisionRecord | null>;
}
