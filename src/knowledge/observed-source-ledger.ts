...
export type RawMeetingNoteSnapshot = {
  schemaVersion: 1;
  title: string | null;
  lifecycle: "not-ready" | "ready" | "failed" | "unknown";
  calendar: {
    startAt: string;
    endAt: string;
    ...
    async record(recordInput) {
      ...
      return input.database.transaction(async (transaction) => {
        ...
        const headResult = await transaction.query<SourceHeadRow>(
          `SELECT current_revision, current_content_hash
        ...
        if (head.current_content_hash === contentHash) {
          ...
          const snapshotResult = await transaction.query<SourceSnapshotRow>(
            `SELECT source_revision, content_hash, provider_version, captured_at, raw_payload_json
