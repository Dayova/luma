...
describe("production AI usage composition", () => {
  it.each([false, true])(
    "serves usage and saves notes at a zero cap without dispatching (shared budget: %s)",
    async (shared) => {
      ...
      const transport: DiscordJsTransport = {
        ...
        resolveChannel: ({ channelId }) =>
          Promise.resolve(channels.get(channelId) ?? null),
      ...
      const app = await startServer(
        {
          ...
          LUMA_AI_MONTHLY_LIMIT_USD: shared ? "30" : "0",
          LUMA_DISCORD_ALLOWED_PARENT_CHANNEL_IDS: "100000000000000002"
        },
        {
          ...(shared
            ? { aiUsageBudget: createAiUsageBudget({ database, monthlyLimitUsd: 0 }) }
            : {}),
